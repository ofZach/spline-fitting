...
LUMatrix<T>& LUMatrix<T>::operator=(const LUMatrix<T>& a){
  ...
  for(int i=0;i<Basic2DArray<T>::rows();++i)
    for(int j=0;j<Basic2DArray<T>::cols();++j)
      Basic2DArray<T>::elem(i,j) = a(i,j) ;
  pivot_ = a.pivot_ ;
  return *this ;
}
...
LUMatrix<T>& LUMatrix<T>::decompose(const Matrix<T>& a)
{
  ...
  nm1 = n - 1;
    ...
    //printf("  pivot_ %i \n", pivot_.size());
    if (pivot_.size() == 1) pivot_.resize(n);
  for (k = 0; k < n; k++)
    pivot_[k] = k ;
  ...
  if (nm1 >= 1)	/* non-trivial problem */
      ...
      for (k = 0; k < nm1; k++)
	{
	  ...
	  if ( Basic2DArray<T>::elem(l,k) != 0.0 )
	    {			// nonsingular pivot found 
	      if (l != k ){	// interchange needed 
		for (i = k; i < n; i++)
		  {
		    t = Basic2DArray<T>::elem(l,i) ;
		    Basic2DArray<T>::elem(l,i) = Basic2DArray<T>::elem(k,i) ;
		    Basic2DArray<T>::elem(k,i) = t ; 
		  }
		sign = -sign ;
	      }
	      q =  Basic2DArray<T>::elem(k,k);	/* scale row */
	      for (i = kp1; i < n; i++)
		{
		  t = - Basic2DArray<T>::elem(i,k)/q;
		  Basic2DArray<T>::elem(i,k) = t;
		  for (j = kp1; j < n; j++)
		    Basic2DArray<T>::elem(i,j) += t * Basic2DArray<T>::elem(k,j);
		}
	    }
	  else		/* pivot singular */
	    errval = k;
  ...
  pivot_[nm1] = nm1;
  if (Basic2DArray<T>::elem(nm1,nm1) == 0.0)
    errval = nm1;  
  return *this;
}
...
T LUMatrix<T>::determinant(){
  T det = Basic2DArray<T>::elem(0,0) ;
  for(int i=1;i<Basic2DArray<T>::rows();++i)
    det *= Basic2DArray<T>::elem(i,i) ;
  return det * (T)sign ;
}
...
void LUMatrix<T>::inverseIn(Matrix<T>& inv) 
{
  ...
  if ( Basic2DArray<T>::rows() != Basic2DArray<T>::cols() )
    {
#ifdef USE_EXCEPTION
    throw WrongSize2D(Basic2DArray<T>::rows(),Basic2DArray<T>::cols(),0,0) ;
#else
      Error error("invm");
      error << "matrix inverse, not square: " << Basic2DArray<T>::rows() << " by " << cols() << endl;
      error.fatal();
#endif
    }

  n = coln = Basic2DArray<T>::rows();
