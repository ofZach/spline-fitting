...
Matrix<T>& Matrix<T>::operator=(const Matrix<T> &a){
  ...
  if ( a.rows() != Basic2DArray<T>::rows() || a.cols() != Basic2DArray<T>::cols() ){
    resize(a.rows(),a.cols()) ;
  }
  
  int sze = Basic2DArray<T>::rows()*Basic2DArray<T>::cols() ;
  T *ptr, *aptr ;
  ptr = Basic2DArray<T>::m-1 ;
  aptr = a.m-1 ;
  
  for (i = sze; i > 0; --i)
    *(++ptr) = *(++aptr) ;
  
  Basic2DArray<T>::by_columns = a.by_columns;
...
void Matrix<T>::submatrix(int sr, int sc, Matrix<T> &a)
{
  int rwz,coz,i,j;
    ...
    if ( Basic2DArray<T>::rows() % a.rows() != 0 || Basic2DArray<T>::cols() % a.cols() != 0 || Basic2DArray<T>::rows() < a.rows() || Basic2DArray<T>::cols() < a.cols() )
    {
#ifdef USE_EXCEPTION
      throw WrongSize2D(Basic2DArray<T>::rows(),Basic2DArray<T>::cols(),a.rows(),a.cols()) ;
#else
  ...
  if ( sr >= Basic2DArray<T>::rows()/a.rows() || sr < 0 || sc >= Basic2DArray<T>::cols()/a.cols() || sc < 0 )
    {
#ifdef USE_EXCEPTION
      throw OutOfBound2D(sr,sc,0,Basic2DArray<T>::rows()/a.rows()-1,0,Basic2DArray<T>::cols()/a.cols()-1) ;
#else
      Error error("Matrix<T>::submatrix");
  ...
  for ( i = a.rows()-1; i >= 0; --i )
    {
      ptr = &Basic2DArray<T>::m[(i+rwz)*Basic2DArray<T>::cols()+coz]-1 ;
      for ( j = a.cols(); j > 0; --j)
	*(++ptr) = *(++aptr) ;
    }  
#endif
...
void Matrix<T>::as(int rw, int cl, Matrix<T>& a)
{
  ...
  if ( (rw + a.rows()) > Basic2DArray<T>::rows() || ( cl + a.cols()) > Basic2DArray<T>::cols()) {
#ifdef USE_EXCEPTION
    throw MatrixErr();
#else
  ...
  aptr = a.m-1 ;
  for ( i = 0; i<a.rows(); ++i) {
    pptr = &Basic2DArray<T>::m[(i+rw)*Basic2DArray<T>::cols()+cl]-1 ;
    for ( j = 0; j < a.cols(); ++j)
      *(++pptr) = *(++aptr);
  }
#endif
...
Matrix<T> Matrix<T>::get(int rw, int cl, int nr, int nc) const
{
  ...
  if ( (rw+nr) > Basic2DArray<T>::rows() || (cl+nc) > Basic2DArray<T>::cols()) {
#ifdef USE_EXCEPTION
    throw MatrixErr();
#else
...
double Matrix<T>::norm(void){
  ...
  int init=0 ;
  T *pptr ;
  pptr = Basic2DArray<T>::m-1 ;
  maxsum = 0 ; // Silence the warning message
  for(i=0;i<Basic2DArray<T>::rows();++i){
    sum = 0 ;
    for ( j = 0; j < Basic2DArray<T>::cols(); ++j) 
      sum += *(++pptr) ;
    if(init)
...
void Matrix<T>::diag(const T a)
{
  ...
  iend = Basic2DArray<T>::rows();
  if ( iend > Basic2DArray<T>::cols() )
    iend = Basic2DArray<T>::cols();
  
  for (i = iend-1; i >=0; --i)
    Basic2DArray<T>::elem(i,i) = a;
...
Vector<T> Matrix<T>::getDiag(){
  int i, iend;
  Vector<T> vec(minimum(Basic2DArray<T>::rows(),Basic2DArray<T>::cols())) ;
  iend = minimum(Basic2DArray<T>::rows(),Basic2DArray<T>::cols());
  for (i = iend-1; i >=0; --i)
      vec[i] = Basic2DArray<T>::elem(i,i);
  return vec ;
}
...
{
  T *p1 ;
  p1 = Basic2DArray<T>::m-1 ;
  const int size = Basic2DArray<T>::rows()*Basic2DArray<T>::cols() ;
...
Matrix<T>& Matrix<T>::operator+=(const Matrix<T> &a)
{
  ...
  T *aptr,*sptr ;
  aptr = a.m - 1 ;
  sptr = Basic2DArray<T>::m - 1 ;
  sze = Basic2DArray<T>::rows()*Basic2DArray<T>::cols() ;
  for (i = sze; i > 0; --i){
      *(++sptr) += *(++aptr) ;
  }
  return *this ;
...
Matrix<T>& Matrix<T>::operator-=(const Matrix<T> &a)
{
  ...
  aptr = a.m - 1 ;
  sptr = Basic2DArray<T>::m - 1 ;
  size = Basic2DArray<T>::rows()*Basic2DArray<T>::cols() ;
  for (i = size; i > 0; --i){
      *(++sptr) -= *(++aptr) ;
  }
  return *this ;
...
{
  int size = Basic2DArray<T>::rows();
  T sum = (T)0;
  
  if ( size > Basic2DArray<T>::cols() )
    size = Basic2DArray<T>::cols();
  
  for (int d = 0; d < size; ++d)
    sum += Basic2DArray<T>::elem(d,d) ;
...
{
  int i, j, r = Basic2DArray<T>::cols(), c = Basic2DArray<T>::rows();
  Matrix<T> adj(r,c);
  
  for (i = 0; i < r; ++i)
    for (j = 0; j < c; ++j)
      adj.elem(i,j) = Basic2DArray<T>::elem(j,i) ;
...
{					
  Matrix<T> f(Basic2DArray<T>::rows(),Basic2DArray<T>::cols()) ;
  for(int i=Basic2DArray<T>::rows()-1;i>=0;--i)
    for(int j=Basic2DArray<T>::cols()-1;j>=0;--j)
      {
	f(i,j) = Basic2DArray<T>::elem(i,Basic2DArray<T>::cols()-j-1);
      }
  return f; 
}
...
{					
  ...
  int i, j;
  const int& r = Basic2DArray<T>::cols();
  const int& c = Basic2DArray<T>::rows();
  Matrix<T> adj(r,c);
...
int Matrix<T>::read(char* filename) {
  ...
  Basic2DArray<T>::resize(r,c) ;
  if(!fin.read((char*)Basic2DArray<T>::m,sizeof(T)*r*c)) return 0 ;
...
int Matrix<T>::write(char* filename) {
  ...
  if(!fout)
    return 0 ;
  int r,c ;
  r = Basic2DArray<T>::rows() ; c = Basic2DArray<T>::cols() ;
  ...
  if(!fout.write((char*)Basic2DArray<T>::m,sizeof(T)*r*c)) return 0 ;
  return 1;
}
...
int Matrix<T>::writeRaw(char* filename) {
  ...
  if(!fout)
    return 0 ;
  if(!fout.write((char*)Basic2DArray<T>::m,sizeof(T)*Basic2DArray<T>::rows()*Basic2DArray<T>::cols())) return 0 ;
  return 1;
}
