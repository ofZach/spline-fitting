...
template <class T> Vector<T>& Vector<T>::operator=(const Vector<T> &b)
{
  if(this==&b)
    ...
    if ( BasicArray<T>::n() != b.n())
    {
      resize(b.n()) ;
    }

  BasicArray<T>::sze = b.n() ;
  T *pa, *pb ;
  pa = BasicArray<T>::x-1 ;
  pb = b.x-1 ;
  for(int i=BasicArray<T>::n();i>0;--i){
    *(++pa) = *(++pb) ;
  }
  return *this;
...
{
  if ( BasicArray<T>::size() != b.size())
    {
      resize(b.size()) ;
    }
  T *ptr ;
  ptr = BasicArray<T>::x - 1 ;
  for(int i=BasicArray<T>::size()-1;i>=0;--i)
     *(++ptr) = b[i] ;
...
{
  const int sz = BasicArray<T>::size(); 
  T *ptr ;
  ptr = BasicArray<T>::x-1 ;
  for (int i = sz; i > 0; --i)
    *(++ptr) = d ;
...
{
  if ( a.size() != BasicArray<T>::size())
    {
#ifdef USE_EXCEPTION
      throw WrongSize(BasicArray<T>::size(),a.size()) ;
#else
...
int Vector<T>::minIndex() const {
  T min = BasicArray<T>::x[0] ;
  int index = 0 ;

  for(int i=1;i<BasicArray<T>::n();i++){
    if(BasicArray<T>::x[i]<=min){
      min = BasicArray<T>::x[i] ;
      index = i ;
    }
  }
  return index ;
...
void Vector<T>::qSort(int M){
  ...
  while(1){
	...
	*(v2) = a ;
      ...
      k=(l+ir) >> 1 ;
      swap(BasicArray<T>::x[k],BasicArray<T>::x[l+1]) ;
      if(BasicArray<T>::x[l+1] > BasicArray<T>::x[ir]){
	swap(BasicArray<T>::x[l+1],BasicArray<T>::x[ir]) ;
      }
      if(BasicArray<T>::x[l]> BasicArray<T>::x[ir]){
	swap(BasicArray<T>::x[l],BasicArray<T>::x[ir]) ;
      }
      if(BasicArray<T>::x[l+1] > BasicArray<T>::x[l]){
	swap(BasicArray<T>::x[l+1],BasicArray<T>::x[l]) ;
      }
      i=l+1 ;
      j=ir ;
      a=BasicArray<T>::x[l] ;
	...
	if(*v1 == *v2)  // both are equal to a...
	  break ;
	swap(BasicArray<T>::x[i],BasicArray<T>::x[j]) ;
      }
      BasicArray<T>::x[l] = BasicArray<T>::x[j] ;
      BasicArray<T>::x[j] = a ;
      jstack += 2 ;
      if(jstack>=Nstack){
...
void Vector<T>::sortIndex(Vector<int>& index, int M) const{
  ...
  while(1){
    if(ir-l<M){ // perform an insertion sort when the array is small enough
      for(j=l+1;j<=ir;++j){
	indext = index[j] ;
	a = BasicArray<T>::x[indext] ;
	for(i=j-1;i>=0;--i){
	  if(BasicArray<T>::x[index[i]] <= a) break ;
	  index[i+1] = index[i] ;
	}
	index[i+1] = indext ;
      }
      ...
      if(BasicArray<T>::x[index[l+1]] > BasicArray<T>::x[index[ir]]){
	swap(index[l+1],index[ir]) ;
      }
      if(BasicArray<T>::x[index[l]]> BasicArray<T>::x[index[ir]]){
	swap(index[l],index[ir]) ;
      }
      if(BasicArray<T>::x[index[l+1]] > BasicArray<T>::x[index[l]]){
	swap(index[l+1],index[l]) ;
      ...
      a=BasicArray<T>::x[indext] ;
      while(1){
	while(BasicArray<T>::x[index[i]] < a) { ++i ; }
	while(BasicArray<T>::x[index[j]] > a) { --j ; }
	if(j<i) break ;
	if(BasicArray<T>::x[index[i]] == BasicArray<T>::x[index[j]])
	  break ;
