...
//  }
//  else{
//      maxU.resize(NurbsSurface<T,N>::P.rows()) ;
//      maxAtU_.resize(NurbsSurface<T,N>::P.rows()) ;
//      for(int i=0;i<NurbsSurface<T,N>::P.rows();++i){
//          if(!maxInfluence(i,NurbsSurface<T,N>::U,NurbsSurface<T,N>::degU,maxAtU_[i]))
//	cerr << "Problem in maxInfluence U!\n" ;
//          maxU[i] = nurbsBasisFun(maxAtU_[i],i,NurbsSurface<T,N>::degU,NurbsSurface<T,N>::U) ;
//    }
//    
//  }
//}
...
//  }
//  else{
//      maxV.resize(NurbsSurface<T,N>::P.cols()) ;
//      maxAtV_.resize(NurbsSurface<T,N>::P.cols()) ;
//      for(int i=0;i<NurbsSurface<T,N>::P.cols();++i){
//          if(!maxInfluence(i,NurbsSurface<T,N>::V,NurbsSurface<T,N>::degV,maxAtV_[i]))
//	cerr << "Problem in maxInfluence V!\n" ;
//          maxV[i] = nurbsBasisFun(maxAtV_[i],i,NurbsSurface<T,N>::degV,NurbsSurface<T,N>::V) ;
//    }
//    
//  }
//}
...
//    NurbsSurface<T,N>::offset(i,j) +=  a / (maxU[i]*maxV[j]) ; 
//    if(NurbsSurface<T,N>::baseLevel_){
//    Point_nD<T,N> vecOffset ; 
//        NurbsSurface<T,N>::vecOffset = NurbsSurface<T,N>::offset(i,j).x()*NurbsSurface<T,N>::ivec(i,j) +
//      NurbsSurface<T,N>::offset(i,j).y()*NurbsSurface<T,N>::jvec(i,j) +
//      NurbsSurface<T,N>::offset(i,j).z()*NurbsSurface<T,N>::kvec(i,j) ;
//    NurbsSurface<T,N>::P(i,j).x() = NurbsSurface<T,N>::baseSurf.ctrlPnts()(i,j).x()+vecOffset.x() ;
//    NurbsSurface<T,N>::P(i,j).y() = NurbsSurface<T,N>::baseSurf.ctrlPnts()(i,j).y()+vecOffset.y() ;
//    NurbsSurface<T,N>::P(i,j).z() = NurbsSurface<T,N>::baseSurf.ctrlPnts()(i,j).z()+vecOffset.z() ;
//  }
//  else
//    NurbsSurface<T,N>::P(i,j) = NurbsSurface<T,N>::offset(i,j) ; 
//}
//
///*!
...
//  int k ; 
//
//  NurbsSurface<T,N>::P = NurbsSurface<T,N>::offset ; 
//
...
//  if(NurbsSurface<T,N>::baseLevel_)
//    for(k=0;k<NurbsSurface<T,N>::P.rows();++k)
//      for(int l=0;l<NurbsSurface<T,N>::P.cols();++l)
//	NurbsSurface<T,N>::P(k,l).w() += T(1) ; 
//
//  int sizeU, sizeV ;
//
//  NurbsSurface<T,N>::sizeU = 2*NurbsSurface<T,N>::degU+3 ; 
//  if(i-NurbsSurface<T,N>::degU-1<0) NurbsSurface<T,N>::sizeU += i-NurbsSurface<T,N>::degU-1 ; 
//  if(i+NurbsSurface<T,N>::degU+1>=NurbsSurface<T,N>::P.rows()) sizeU -= i+NurbsSurface<T,N>::degU+1-NurbsSurface<T,N>::P.rows() ;
//
//  NurbsSurface<T,N>::sizeV = 2*NurbsSurface<T,N>::degV+3 ;
//  if(j-NurbsSurface<T,N>::degV-1<0) sizeV += j-NurbsSurface<T,N>::degV-1 ; 
//  if(j+NurbsSurface<T,N>::degV+1>=NurbsSurface<T,N>::P.cols()) sizeV -= j+NurbsSurface<T,N>::degV+1-NurbsSurface<T,N>::P.cols() ;
//  
//  Vector<T> u(sizeU) ;
//  Vector<T> v(sizeV) ;
...
//  int nu = 0 ;
//  int nv = 0 ; 
//  for(k=i-NurbsSurface<T,N>::degU-1;k<=i+NurbsSurface<T,N>::degU+1;++k){
//    if(k<0)
//      continue ;
//    if(k>=NurbsSurface<T,N>::P.rows())
//      break ; 
//    nv = 0 ;
//    for(int l=j-NurbsSurface<T,N>::degV-1;l<=j+NurbsSurface<T,N>::degV+1;++l){
//      if(l<0)
//	continue ;
//      if(l>=NurbsSurface<T,N>::P.cols())
//	break ; 
//      if( k == i && j==l){
//	pts[n].x() = a.x() ; 
//	pts[n].y() = a.y() ; 
...
//    NurbsSurface<T,N>::offset = NurbsSurface<T,N>::P ; 
//    // an offset shouldn't have a weight value.
//    if(NurbsSurface<T,N>::baseLevel_)
//      for(k=0;k<NurbsSurface<T,N>::P.rows();++k)
//	for(int l=0;l<NurbsSurface<T,N>::P.cols();++l)
//	  NurbsSurface<T,N>::offset(k,l).w() -= T(1) ; 
//  }
//  updateSurface(); 
//}
//
...
//  HNurbsSurfaceSP<T,N> *newLevel ;
//
//  if(NurbsSurface<T,N>::nextLevel_)
//    return 0 ;
//
...
//  NurbsSurface<T,N>::rU = nS.rU ;
//  NurbsSurface<T,N>::rV = nS.rV ;
//  offset = nS.offset ;
//
//  updateMaxUV() ; 
//
