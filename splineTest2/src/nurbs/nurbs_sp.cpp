...
void NurbsCurveSP<T,N>::updateMaxU() {
    ...
    error.fatal() ;
      ...
      maxU.resize(NurbsCurve<T,N>::P.n()) ;
      maxAt_.resize(NurbsCurve<T,N>::P.n()) ;
      for(int i=0;i<NurbsCurve<T,N>::P.n();++i){
      if(!maxInfluence(i,NurbsCurve<T,N>::U,NurbsCurve<T,N>::deg_,maxAt_[i]))
	...
	if(maxAt_[i]<maxAt_[i-1]){
      ...
      maxU[i] = basisFun(maxAt_[i],i,NurbsCurve<T,N>::deg_) ;
    }
...
void NurbsCurveSP<T,N>::modOnlySurfCPby(int i, const HPoint_nD<T,N>& a){
  Vector<T> u(2*NurbsCurve<T,N>::deg_+3) ;
  Vector< Point_nD<T,N> > pts(2*NurbsCurve<T,N>::deg_+3) ; 

  int n=0;
  for(int j=i-NurbsCurve<T,N>::deg_-1;j<=i+NurbsCurve<T,N>::deg_+1;++j){
    if(j<0)
      continue ;
    if(j>=NurbsCurve<T,N>::P.n())
      break ; 
    u[n] = maxAt_[j] ;
    if( j == i){
      pts[n].x() = a.x() ; 
  ...
  NurbsCurve<T,N>::movePoint(u,pts) ;
}
