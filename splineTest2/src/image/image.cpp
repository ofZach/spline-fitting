...
//  int i,j ;
//  double mx,b ;
//  if(i1<0 || j1<0 || i1>Basic2DArray<T>::rows() || j1>=Basic2DArray<T>::cols()  ){
//#ifdef USE_EXCEPTION
//    throw OutOfBound2D(i1,j1,0,Basic2DArray<T>::rows()-1,0,Basic2DArray<T>::cols()-1) ;
//#else
...
//    return ;
//  }
//  if(i2 <0 || j2<0 || i2>Basic2DArray<T>::rows() || j2>=Basic2DArray<T>::cols() ){
//#ifdef USE_EXCEPTION
//    throw OutOfBound2D(i2,j2,0,Basic2DArray<T>::rows()-1,0,Basic2DArray<T>::cols()-1) ;
//#else
//    Error error("MatrixImage<T>::drawLine") ;
//    error << "Error in drawing line\n Invalid index ("<< i1 << ", " << j1 << ") to ( " << i2 << ", " << j2 << ") \n" ;
//    error.warning() ;
...
void MatrixImage<T>::drawPoint(int i, int j, double r , T color){
  for(int y=i-int(ceil(r)) ; y<i+int(ceil(r)) ; y++)
    for(int x = j-int(ceil(r)) ; x<j+int(ceil(r)) ; x++){
      if(y>=0 && y<Basic2DArray<T>::rows() && x>=0 && x<Basic2DArray<T>::cols()){
...
void MatrixImage<T>::store(Matrix<T>& a){
  if(a.rows() != Basic2DArray<T>::rows() || a.cols() != Basic2DArray<T>::cols()) {
    a.resize(Basic2DArray<T>::rows(),Basic2DArray<T>::cols()) ;
  }
  T *aptr, *bptr ;
  int size,i ;
  aptr = &a(0,0)-1 ;
  bptr = Basic2DArray<T>::m-1 ;
  size = Basic2DArray<T>::cols()*Basic2DArray<T>::rows() ;
  for(i=0;i<size;i++)
    *(++aptr) = *(++bptr) ;  
}
